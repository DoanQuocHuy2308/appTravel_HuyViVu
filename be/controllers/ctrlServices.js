...
exports.createServices = async (req, res) => {
	try {
		...
		if (!name || !service_type || !description || !price) {
			return res.status(400).json({ message: 'Vui lòng cung cấp đầy đủ thông tin bắt buộc' });
		}
		const servicesId = await Services.createServices( name, service_type, description, price );
...
exports.updateServices = async (req, res) => {
	try {
		...
		const updatedServices = await Services.updateServices(id,name, service_type, description, price);
		if (!updatedServices){
